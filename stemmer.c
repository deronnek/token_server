...
{  switch (z->b[z->k-1])
   {  case 'a': if (ends(z, "\02" "al")) break; return;
      ...
      case 'n': if (ends(z, "\03" "ant")) break;
                ...
                /* KDR added test for z->j > -1 */
      case 'o': if (ends(z, "\03" "ion") && z->j > -1 && (z->b[z->j] == 's' || z->b[z->j] == 't')) break;
...
extern int stem(struct stemmer * z, char * b, int k)
{
   ...
   /* With this line, strings of length 1 or 2 don't go through the
      ...
      algorithm. */
     //printf("%s %d\n",z->b,z->k);
     /* Separating the steps saves us from a bug where we hand in something 
     like 'ies', which sets z->k to 0, and in step 2 we try to access z->-1 */
     step1ab(z); 
     if(z->k <= 1) return z->k;
     step1c(z); 
     if(z->k <= 1) return z->k;
